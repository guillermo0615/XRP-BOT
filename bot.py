...
1. ATR stop loss: 2. Take profit: 3. Trailing stop: entry - (2.0 x ATR)
entry + (3.5 x ATR) → 1.75:1 R/R
activates at +1.5x ATR gain, locks at entry + 0.5x ATR
...
Timeframe: """
15-minute candles, loop every 15 minutes
...
ATR_TARGET_MULT = 3.5 # take profit = entry + (3.5 x ATR)
ATR_TRAIL_MULT = 1.5 # activate trailing stop after 1.5x ATR gain
ATR_TRAIL_LOCK = 0.5 # lock trail stop at entry + 0.5x ATR
VOLUME_PERIOD = 20
...
r.raise_for_status()
return r.json()
# ── Market Data (Kraken — public, no API key needed) ─────────────────────────
def fetch_candles():
"""
Pulls 15-min OHLCV candles for XRP/USD from Kraken's free public API.
Returns the last ~720 candles (Kraken default).
"""
...
# Kraken key for XRP/USD is XXRPZUSD
...
# Drop the last (incomplete/live) candle
df = df.iloc[:-1].reset_index(drop=True)
return df
def get_price():
"""Mid price from Robinhood best bid/ask."""
...
log(f"POSITION OPEN | Entry={price:.4f} | SL={pos['stop']:.4f} "
f"| TP={pos['target']:.4f} | ATR={atr_val:.4f}")
...
log(f"SL: {ATR_STOP_MULT}x ATR | TP: {ATR_TARGET_MULT}x ATR "
f"| Trailing: {ATR_TRAIL_MULT}x ATR")
log("=" * 60)
# Sync existing position on startup
held = get_holdings()
if held > 0.01:
price = get_price()
