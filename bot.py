...
def manage_mr(price, adx, atr_ratio, fng_today):
    global cooldown_mr
    pos_mr["bars_held"] += 1
    d   = pos_mr["direction"]
    ...
    if hit_stop:
        ...
        log_trade("MR", d, pos_mr["entry"], price, pct, "stop_loss",
                  pos_mr["bars_held"], fng_today, adx, atr_ratio)
        pos_mr["active"] = False
        ...
        log_trade("MR", d, pos_mr["entry"], price, pct, "time_exit",
                  pos_mr["bars_held"], fng_today, adx, atr_ratio)
        pos_mr["active"] = False
        cooldown_mr = COOLDOWN_MR   # thesis didn't play out — same as stop loss
    else:
...
def manage_tf(price, ema9, ema21, prev_ema9, prev_ema21, rsi_15, current_atr, adx, atr_ratio, fng_today):
    global cooldown_tf
    pos_tf["bars_held"] += 1
    d    = pos_tf["direction"]
    ...
    def close_tf(reason, with_cooldown=False):
        ...
        log_trade("TF", d, pos_tf["entry"], price, pct, reason,
                  pos_tf["bars_held"], fng_today, adx, atr_ratio)
        pos_tf["active"] = False
        if with_cooldown:
            global cooldown_tf
...
def manage_ss(price, adx, atr_ratio, fng_today):
    pos_ss["bars_held"] += 1
    d   = pos_ss["direction"]
    ...
    def close_ss(reason):
        ...
        log_trade("SS", d, pos_ss["entry"], price, pct, reason,
                  pos_ss["bars_held"], fng_today, adx, atr_ratio)
        pos_ss["active"] = False
    if stop_hit:
        close_ss("stop_loss")
    elif target_hit:
...
def manage_bm(price, adx, atr_ratio, fng_today):
    pos_bm["bars_held"] += 1
    ...
    def close_bm(reason):
        ...
        log_trade("BM", "long", pos_bm["entry"], price, pct, reason,
                  pos_bm["bars_held"], fng_today, adx, atr_ratio)
        pos_bm["active"] = False
    if stop_hit:
...
def run():
    ...
    while True:
        try:
            ...
            if pos_mr["active"]:
                manage_mr(price, adx, atr_ratio, fng_today)
            ...
            if pos_tf["active"]:
                manage_tf(price, ema9, ema21, prev_ema9, prev_ema21, rsi_15, atr_v, adx, atr_ratio, fng_today)
            elif cooldown_tf == 0:
            ...
            if pos_ss["active"]:
                manage_ss(price, adx, atr_ratio, fng_today)
            else:
            ...
            if pos_bm["active"]:
                manage_bm(price, adx, atr_ratio, fng_today)
            else:
