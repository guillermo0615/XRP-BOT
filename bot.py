...
def manage_tf(price, ema9, ema21, prev_ema9, prev_ema21, rsi_15):
    ...
    elif pos_tf["bars_held"] >= TIME_EXIT_TF:
        close_tf(f"Time exit {TIME_EXIT_TF} bars")
...
def manage_bm(price):
    ...
    elif pos_bm["bars_held"] >= TIME_EXIT_BM:
        close_bm(f"Time exit {TIME_EXIT_BM} bars")
    else:
        log(f"[BM] Holding | Bar {pos_bm['bars_held']}/{TIME_EXIT_BM} | PnL={pct:+.2f}% | Trail={pos_bm['trail_stop']:.4f} | SL={pos_bm['stop']:.4f}")
    ...
    while True:
        try:
            ...
            if pos_mr["active"]:
                ...
                elif mr_oversold and FNG_MR_MIN <= fng_today <= FNG_MR_MAX and rsi_1h > RSI_1H_MIN:
                ...
                elif mr_overbought and FNG_MR_MIN <= fng_today <= FNG_MR_MAX and rsi_1h < RSI_1H_MAX:
            ...
            if pos_tf["active"]:
            ...
            elif cooldown_tf == 0 and regime == "TRENDING":
                ...
                if not vol_ok:
                ...
                elif ema_bull and rsi_ok_tf and macd_bull and fng_today >= FNG_TF_MIN:
                    ...
                    log(f"[TF] SHORT SIGNAL | EMA bear cross | MACD bearish | RSI={rsi_15:.1f} | FNG={fng_today}")
                ...
                log(f"[TF] SKIP - Ranging ADX={adx:.1f}")
            ...
            if pos_bm["active"]:
                manage_bm(price)
            elif not pos_mr["active"] and not pos_tf["active"]:
                ...
                log("[BM] SKIP - MR or TF position active")
