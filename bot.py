...
# ── Monotonic nonce ─────────────────────────────────────────────────────────
# Kraken requires every private call's nonce to be strictly greater than the
# previous one. time.time()*1000 can collide when two calls land in the same
# millisecond (e.g. balance read then order in one loop). Seed once from the
# clock, then increment on every call so nonces always rise.
...
# ── Order execution ─────────────────────────────────────────────────────────
def query_fill(txid):
    # Market orders usually settle in <1s, but not always instantly.
    # Returns (avg_fill_price, executed_qty) from Kraken's own record.
    for _ in range(5):
        ...
        if o and o["status"] == "closed":
            ...
            # cost/vol_exec is the true average fill price across any partials;
            # fall back to o["price"] if cost is missing.
...
def place_order(side, volume, leverage=None):
    ...
    log(f"ORDER {side.upper()} filled {fill_qty:.6f} XRP @ {fill_price:.4f} leverage={leverage} txid={txid}")
...
def calc_rsi(s, n=14):
    ...
    rs = g / (l + 1e-9)   # epsilon: avoid inf/nan in one-directional moves
    return 100 - (100 / (1 + rs))
...
def calc_adx(df, n=14):
    ...
    atr_s     = calc_atr(df, n)
    # Wilder smoothing (alpha=1/n) matches TradingView/TC2000
    alpha     = 1.0 / n
...
def empty_pos():
    return {
        ...
        "target":         0.0,
        "bars_held":      0,
        "entry_bar_time": 0,     # candle timestamp at open; bars_held derived from this
        "trail_active":   False,
        "trail_stop":     0.0,
        "atr_at_entry":   0.0,
    }
...
cooldown_mr_until = 0   # candle timestamp until which MR is in cooldown
cooldown_tf_until = 0   # candle timestamp until which TF is in cooldown

def log(msg):
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] {msg}", flush=True)

# ── Trade logger ─────────────────────────────────────────────────────────────
# Railway's filesystem is ephemeral — writes to /tmp which resets on restart.
# Every closed trade is ALSO printed as a TRADE_RECORD line to stdout so the
# Railway log history (which Railway does persist) is your permanent backup.
# To review: copy all lines containing TRADE_RECORD from Railway logs and
# paste them here — the bot will parse and summarize them automatically.
TRADE_LOG_PATH = "/tmp/trade_log.json"

def log_trade(strategy, direction, entry_price, exit_price, pct_gain,
              exit_reason, bars_held, fng_at_entry, adx_at_entry, atr_ratio_at_entry):
    record = {
        ...
        "adx_at_entry":      round(adx_at_entry, 2),
        "atr_ratio_at_entry": round(atr_ratio_at_entry, 3),
        "win":               pct_gain > 0,
    }
    # Print to stdout first — Railway log history is the durable backup
    print(f"[TRADE_RECORD] {json.dumps(record)}", flush=True)
    # Also write/append to local JSON file for the current session
    try:
        try:
        ...
        log(f"[LOG] Trade recorded — {strategy} {direction} {pct_gain:+.2f}% via {exit_reason} | Total trades logged: {len(existing)}")
    except Exception as e:
        log(f"[LOG] WARNING: could not write trade log: {e}")

def print_trade_summary():
    """Print a summary of all trades logged this session."""
    try:
        with open(TRADE_LOG_PATH, "r") as f:
            trades = json.load(f)
        if not trades:
            log("[LOG] No trades logged this session yet.")
            return
        log(f"[LOG] ── SESSION TRADE SUMMARY ({len(trades)} trades) ──────────────")
        by_strategy = {}
        for t in trades:
            s = t["strategy"]
        ...
        for s, ts in by_strategy.items():
            ...
            losses = len(ts) - wins
            avg    = sum(t["pct_gain"] for t in ts) / len(ts)
            total  = sum(t["pct_gain"] for t in ts)
            log(f"[LOG]   {s}: {len(ts)} trades | {wins}W {losses}L | Avg={avg:+.2f}% | Total={total:+.2f}%")
            for t in ts:
                log(f"[LOG]     {t['timestamp']} | {t['direction'].upper()} | {t['pct_gain']:+.2f}% | {t['exit_reason']} | {t['bars_held']} bars | FNG={t['fng_at_entry']}")
        log("[LOG] ─────────────────────────────────────────────────────────")
    except (FileNotFoundError, json.JSONDecodeError):
        log("[LOG] No trade log file found for this session.")

# ── MR management ───────────────────────────────────────────────────────────
def open_mr_long(fill_price, fill_qty, atr_val, bb_mid, bar_time):
    pos_mr.update({
        "active": True, "direction": "long",
        "entry": fill_price, "qty": fill_qty,
        "stop":   fill_price - (ATR_STOP_MR * atr_val),
        "target": bb_mid,
        "bars_held": 0, "entry_bar_time": bar_time,
        "trail_active": False, "trail_stop": 0.0, "atr_at_entry": atr_val,
    })
    log(f"[MR] LONG OPEN | Entry={fill_price:.4f} | SL={pos_mr['stop']:.4f} | TP={pos_mr['target']:.4f} | Qty={fill_qty:.6f}")

def open_mr_short(fill_price, fill_qty, atr_val, bb_mid, bar_time):
    pos_mr.update({
    ...
    log(f"[MR] SHORT OPEN | Entry={fill_price:.4f} | SL={pos_mr['stop']:.4f} | TP={pos_mr['target']:.4f} | Qty={fill_qty:.6f}")

def manage_mr(price, adx, atr_ratio, fng_today, current_bar_time):
    global cooldown_mr_until
    d   = pos_mr["direction"]
    # bars_held derived from real candle time — immune to loop sleep interval
    pos_mr["bars_held"] = int((current_bar_time - pos_mr["entry_bar_time"]) / 900)
    bars_held = pos_mr["bars_held"]
    ...
    if hit_stop:
        ...
        log_trade("MR", d, pos_mr["entry"], price, pct, "stop_loss",
                  bars_held, fng_today, adx, atr_ratio)
        pos_mr["active"] = False
        cooldown_mr_until = current_bar_time + COOLDOWN_MR * 900
    elif hit_target:
        sell_spot(pos_mr["qty"], "[MR-L]") if d == "long" else buy_cover(pos_mr["qty"], "[MR-S]")
        log(f"[MR] CLOSED {d.upper()} - Take profit | PnL={pct:+.2f}%")
        log_trade("MR", d, pos_mr["entry"], price, pct, "take_profit",
                  bars_held, fng_today, adx, atr_ratio)
        pos_mr["active"] = False
    elif bars_held >= TIME_EXIT_MR:
        sell_spot(pos_mr["qty"], "[MR-L]") if d == "long" else buy_cover(pos_mr["qty"], "[MR-S]")
        log(f"[MR] CLOSED {d.upper()} - Time exit | PnL={pct:+.2f}%")
        log_trade("MR", d, pos_mr["entry"], price, pct, "time_exit",
                  bars_held, fng_today, adx, atr_ratio)
        pos_mr["active"] = False
        cooldown_mr_until = current_bar_time + COOLDOWN_MR * 900   # thesis didn't play out — same as stop loss
    else:
        log(f"[MR] Holding {d.upper()} | Bar {bars_held}/{TIME_EXIT_MR} | PnL={pct:+.2f}% | SL={pos_mr['stop']:.4f} | TP={pos_mr['target']:.4f}")

# ── TF management ───────────────────────────────────────────────────────────
def open_tf_long(fill_price, fill_qty, atr_val, bar_time):
    pos_tf.update({
        "active": True, "direction": "long",
        "entry": fill_price, "qty": fill_qty,
        "stop":   fill_price - (ATR_STOP_TF * atr_val),
        "target": fill_price + (ATR_TARGET_TF * atr_val),
        "bars_held": 0, "entry_bar_time": bar_time,
        "trail_active": False, "trail_stop": 0.0, "atr_at_entry": atr_val,
    })
    log(f"[TF] LONG OPEN | Entry={fill_price:.4f} | SL={pos_tf['stop']:.4f} | TP={pos_tf['target']:.4f} | Qty={fill_qty:.6f}")

def open_tf_short(fill_price, fill_qty, atr_val, bar_time):
    pos_tf.update({
        "active": True, "direction": "short",
        "entry": fill_price, "qty": fill_qty,
        "stop":   fill_price + (ATR_STOP_TF_SH * atr_val),
        "target": fill_price - (ATR_TARGET_TF * atr_val),
        "bars_held": 0, "entry_bar_time": bar_time,
        "trail_active": False, "trail_stop": 0.0, "atr_at_entry": atr_val,
    })
    log(f"[TF] SHORT OPEN | Entry={fill_price:.4f} | SL={pos_tf['stop']:.4f} | TP={pos_tf['target']:.4f} | Qty={fill_qty:.6f}")

def manage_tf(price, ema9, ema21, prev_ema9, prev_ema21, rsi_15, current_atr, adx, atr_ratio, fng_today, current_bar_time):
    global cooldown_tf_until
    d    = pos_tf["direction"]
    pos_tf["bars_held"] = int((current_bar_time - pos_tf["entry_bar_time"]) / 900)
    bars_held = pos_tf["bars_held"]
    gain = price - pos_tf["entry"] if d == "long" else pos_tf["entry"] - price
    pct  = (gain / pos_tf["entry"]) * 100
    # Activate trailing stop once gain reaches ATR_TRAIL_MULT (threshold uses entry ATR)
    ...
    # Ratchet uses current ATR each bar so trail distance adapts to live volatility
    if pos_tf["trail_active"]:
        if d == "long":
            new_trail = price - (ATR_TRAIL_MULT * current_atr)
    ...
    def close_tf(reason, with_cooldown=False):
        if d == "long":
            sell_spot(pos_tf["qty"], "[TF-L]")
        else:
            buy_cover(pos_tf["qty"], "[TF-S]")
        log(f"[TF] CLOSED {d.upper()} - {reason} | PnL={pct:+.2f}%")
        log_trade("TF", d, pos_tf["entry"], price, pct, reason,
                  bars_held, fng_today, adx, atr_ratio)
        pos_tf["active"] = False
        if with_cooldown:
            global cooldown_tf_until
            cooldown_tf_until = current_bar_time + COOLDOWN_TF * 900
    if trail_hit:
        close_tf("trailing_stop", with_cooldown=True)
    elif stop_hit:
        close_tf("stop_loss", with_cooldown=True)
    elif target_hit:
        close_tf("take_profit")
    elif d == "long" and ema_bearish:
        close_tf(f"ema_cross_bearish_rsi{rsi_15:.0f}")
    elif d == "short" and ema_bullish:
        close_tf(f"ema_cross_bullish_rsi{rsi_15:.0f}")
    else:
...
# ── SS management ───────────────────────────────────────────────────────────
def open_ss_long(fill_price, fill_qty, bar_time):
    pos_ss.update({
        "active": True, "direction": "long",
        "entry": fill_price, "qty": fill_qty,
        "stop":   fill_price * (1 - SS_STOP_PCT),
        "target": fill_price * (1 + SS_TARGET_PCT),
        "bars_held": 0, "entry_bar_time": bar_time,
        "trail_active": False, "trail_stop": 0.0, "atr_at_entry": 0.0,
    })
    log(f"[SS] LONG OPEN | Entry={fill_price:.4f} | SL={pos_ss['stop']:.4f} | TP={pos_ss['target']:.4f} | Qty={fill_qty:.6f}")

def open_ss_short(fill_price, fill_qty, bar_time):
    pos_ss.update({
        "active": True, "direction": "short",
        "entry": fill_price, "qty": fill_qty,
        "stop":   fill_price * (1 + SS_STOP_PCT),
    ...
    log(f"[SS] SHORT OPEN | Entry={fill_price:.4f} | SL={pos_ss['stop']:.4f} | TP={pos_ss['target']:.4f} | Qty={fill_qty:.6f}")

def manage_ss(price, adx, atr_ratio, fng_today, current_bar_time):
    d   = pos_ss["direction"]
    pos_ss["bars_held"] = int((current_bar_time - pos_ss["entry_bar_time"]) / 900)
    bars_held = pos_ss["bars_held"]
    ...
    def close_ss(reason):
        ...
        log_trade("SS", d, pos_ss["entry"], price, pct, reason,
                  bars_held, fng_today, adx, atr_ratio)
        pos_ss["active"] = False
    if stop_hit:
        close_ss("stop_loss")
    elif target_hit:
        close_ss("take_profit")
    elif bars_held >= TIME_EXIT_SS:
        close_ss("time_exit")
    else:
        log(f"[SS] Holding {d.upper()} | Bar {bars_held}/{TIME_EXIT_SS} | PnL={pct:+.2f}% | SL={pos_ss['stop']:.4f} | TP={pos_ss['target']:.4f}")

# ── BM management ───────────────────────────────────────────────────────────
def open_bm_long(fill_price, fill_qty, atr_val, bar_time):
    pos_bm.update({
        "active": True, "direction": "long",
        "entry":      fill_price,
        "qty":        fill_qty,
        "stop":       fill_price - (BM_ATR_STOP * atr_val),
        "target":     0.0,
        "trail_stop": fill_price - (BM_ATR_TRAIL * atr_val),
        "trail_active": True,
        "bars_held":  0,
        "entry_bar_time": bar_time,
        "atr_at_entry": atr_val,
    })
    log(f"[BM] LONG OPEN | Entry={fill_price:.4f} | SL={pos_bm['stop']:.4f} | Trail={pos_bm['trail_stop']:.4f} | Qty={fill_qty:.6f}")

def manage_bm(price, adx, atr_ratio, fng_today, current_bar_time):
    pos_bm["bars_held"] = int((current_bar_time - pos_bm["entry_bar_time"]) / 900)
    bars_held = pos_bm["bars_held"]
    pct       = ((price - pos_bm["entry"]) / pos_bm["entry"]) * 100
    # Only ratchet trail after 3-bar noise window — prevents the trail from
    # tightening during bars 1-3 and triggering a premature exit on bar 4
    if bars_held > 3:
    ...
    trail_hit = price <= pos_bm["trail_stop"] and bars_held > 3
    def close_bm(reason):
        sell_spot(pos_bm["qty"], "[BM]")
        log(f"[BM] CLOSED - {reason} | PnL={pct:+.2f}%")
        log_trade("BM", "long", pos_bm["entry"], price, pct, reason,
                  bars_held, fng_today, adx, atr_ratio)
        pos_bm["active"] = False
    if stop_hit:
        close_bm("hard_stop_loss")
    elif trail_hit:
        close_bm("trailing_stop")
    else:
        log(f"[BM] Holding | Bar {bars_held} | PnL={pct:+.2f}% | Trail={pos_bm['trail_stop']:.4f} | SL={pos_bm['stop']:.4f}")

# ── Main loop ───────────────────────────────────────────────────────────────
def run():
    global cooldown_mr_until, cooldown_tf_until
    ...
    print_trade_summary()  # show any trades logged before a restart
    ...
    while True:
        try:
            ...
            last  = df15.iloc[-1]
            prev  = df15.iloc[-2]
            bar_time = int(last["time"])   # timestamp of last CLOSED candle; basis for bars_held
            price = get_price()
            ...
            # SS cross detection uses API's own yesterday value — reliable daily signal
            # (prev_fng loop variable was broken: updated every 15min but FNG changes daily,
            # so after first iteration prev_fng == fng_today and cross never fired again)
            ...
            mr_cooldown_active = bar_time < cooldown_mr_until
            tf_cooldown_active = bar_time < cooldown_tf_until
            if mr_cooldown_active:
                remaining = int((cooldown_mr_until - bar_time) / 900)
                log(f"[MR] Cooldown: ~{remaining} bars remaining")
            if tf_cooldown_active:
                remaining = int((cooldown_tf_until - bar_time) / 900)
                log(f"[TF] Cooldown: ~{remaining} bars remaining")

            # ── STRATEGY 1: Mean Reversion ──────────────────────────────────
            if pos_mr["active"]:
                manage_mr(price, adx, atr_ratio, fng_today, bar_time)
            elif not mr_cooldown_active and regime == "RANGING":
                ...
                if not atr_ok:
                    ...
                    fill_price, fill_qty = buy_spot(TRADE_AMOUNT_MR, "[MR]")
                    open_mr_long(fill_price, fill_qty, atr_v, bb_mid, bar_time)
                elif mr_overbought and FNG_MR_SHORT_MIN <= fng_today <= FNG_MR_MAX and rsi_1h < RSI_1H_SHORT_MAX:
                    log(f"[MR] SHORT SIGNAL | Price at upper band | RSI={rsi_15:.1f} | FNG={fng_today}")
                    fill_price, fill_qty = sell_short(TRADE_AMOUNT_MR, "[MR]")
                    open_mr_short(fill_price, fill_qty, atr_v, bb_mid, bar_time)
                else:
                    log("[MR] Watching - no signal")
            elif not mr_cooldown_active:
                log(f"[MR] SKIP - Trending ADX={adx:.1f}")

            # ── STRATEGY 2: Trend Following ─────────────────────────────────
            if pos_tf["active"]:
                manage_tf(price, ema9, ema21, prev_ema9, prev_ema21, rsi_15, atr_v, adx, atr_ratio, fng_today, bar_time)
            elif not tf_cooldown_active:
                ...
                if not vol_ok:
                    ...
                    fill_price, fill_qty = buy_spot(TRADE_AMOUNT_TF, "[TF]")
                    open_tf_long(fill_price, fill_qty, atr_v, bar_time)
                    ...
                    fill_price, fill_qty = sell_short(TRADE_AMOUNT_TF, "[TF]")
                    open_tf_short(fill_price, fill_qty, atr_v, bar_time)
            ...
            if pos_ss["active"]:
                manage_ss(price, adx, atr_ratio, fng_today, bar_time)
            else:
                ...
                if fng_cross_fear and vol_ok and price > ema9_1h:
                    ...
                    fill_price, fill_qty = buy_spot(TRADE_AMOUNT_SS, "[SS]")
                    open_ss_long(fill_price, fill_qty, bar_time)
                    ...
                    fill_price, fill_qty = sell_short(TRADE_AMOUNT_SS, "[SS]")
                    open_ss_short(fill_price, fill_qty, bar_time)
            ...
            if pos_bm["active"]:
                manage_bm(price, adx, atr_ratio, fng_today, bar_time)
            else:
                ...
                if fng_today < FNG_BM_MIN:
                    ...
                    fill_price, fill_qty = buy_spot(TRADE_AMOUNT_BM, "[BM]")
                    open_bm_long(fill_price, fill_qty, atr_v, bar_time)
